...
// Local OCR tool: run with `go run ocr/scripts/ocr_local.go <image.png>`
// Uses the phonefast OCR engine directly on a local image file.
package main
...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintf(os.Stderr, "Usage: go run ocr/scripts/ocr_local.go <image.png>\n")
		os.Exit(1)
	}
	imagePath := os.Args[1]
