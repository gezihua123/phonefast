...
// Returns ErrNotAvailable if the engine cannot be initialized
// (models not downloaded, runtime library missing, or an ncnn build
// without the ncnn tag / model paths).
func (s *Service) Recognize(pngImage []byte) ([]TextResult, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Service) getEngine() (Engine, error) {
	if s.eng != nil {
		return s.eng, nil
	}
	if s.initErr != nil {
