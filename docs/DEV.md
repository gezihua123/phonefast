...
> **口径说明**：下表所有时间为同一轮测试的逐步骤 `time.Now()` 分解，加和等于引擎总耗时。
> "端到端" = 引擎 + 截图 + daemon IPC，必然 > 引擎，与分解不直接相加。

```
引擎: ONNX Runtime CPU (brew 1.27.1, 无 BLAS 优化)
模型: PP-OCR v3 det (2.4MB) + rec (10.7MB)
检测: maxSide=1024 降采样
识别: H=40 (从 48 降, 9% 提速, 中英文质量 0 损失)

macOS Vision ON 路径 (20 图 × 15 轮基准):
  Vision ANE detect:    <1ms   (ANE 硬件)
  Det 前处理:            0ms   (Vision 跳过 ONNX det)
  Rec ONNX 推理:        ~50ms   (14 boxes × 5.0ms, 全流程 ~70%)
  Rec 前处理:            ~8ms   (crop + pad H=40)
  CTC 解码:              ~5ms
  PNG decode:             ~6ms
  ─────────────────────
  纯 OCR 引擎 (基准):    70ms
  + 截图 + daemon IPC:   ~30ms
  ─────────────────────
  真机端到端:           ~100ms (稀疏) ~ 200ms (密集 34 box)

纯 ONNX 路径 (Linux/Windows 同构):
  Det ONNX 推理:        18ms   (448×1024, maxSide=1024 降采样)
  Det 前处理:            19ms  (bilinear resize, 直连 Pix)
  Rec + 前处理 + CTC:    73ms   (同上)
  PNG decode:            10ms
  ─────────────────────
  纯 OCR 引擎:          ~120ms
...
从原始 330ms 到最终 110ms 的完整优化记录（同一台设备、同一屏幕）：
...
| 3 | dilateMask 替代 O(n²) mergeBoxes | 186ms | — |
| 4 | det 直连 Pix (替换 pixelChannel 虚调用) | 186ms | — |
| 5 | rec 批量推理 (batch 替代逐个) | 186ms | — |
| 6 | **macOS Vision ANE detect** | **110ms** | **3.0×** |
| 7 | bubble sort → sort.Slice, 去死代码 | 108ms | 3.1× |
| 8 | Rec H 选型测试 (48/40/32) → 确认 H=48 最准 | — | 回退 |

> 阶段 3-5 端到端无变化：这些优化抵消了 rec 批量推理引入的输出拷贝开销，
> 但为 Vision 路径（阶段6）打下了 rec 复用基础。
...
```
coremltools 9.0:  砍掉 ONNX converter
coremltools 8.2:  无 converters.onnx
coremltools 7.2:  无 converters.onnx
onnx-coreml 独立: 依赖 coremltools.converters.nnssa (5.x已删)
ONNX→PyTorch→CoreML: onnx2torch 无法处理 PaddlePaddle 非标准 ONNX ops
```
...
PP-OCR rec 模型原始训练输入为 H=48×W=320。降低 H 可减少推理像素数，
从而提速，但可能影响字符识别质量。

#### 测试方法

1. **速度对比**: 同一张图(34 boxes)用 H=48/40/32 测纯 ONNX rec 推理
2. **中文质量**: 10 张中文截图逐图对比 H=48 vs H=40 vs H=32 的识别结果
3. **全量验证**: 20 张图 (10CN+10EN) × 15 轮 = 300 推理基准

#### 速度数据

```
H=48:  132ms rec 推理,  3.9ms/box  (基线)
H=40:  121ms rec 推理,  3.6ms/box  (-8%)
H=32:  102ms rec 推理,  3.0ms/box  (-23%)
```
...
- **H=40 质量有涨有跌**，非零损失：
  - 部分词丢失（"搜索设**置**" → "搜索设"，"**存**储" → "y储"）
  - 部分词改善（"提示音和" → "提示音和**振动**"，H=48 反而漏了）
  - 状态栏小字在所有 H 值下都不稳定（图标干扰），不是 H 导致的问题
- **H=32** 丢字明显增多（"搜索设置"→"搜素设"，"添加其他"→"加其他"）
- **结论：H=48 是唯一准确率最优值。最终选定 H=48。**
  H=40 损失 10.1% 识别字符（10图总量: 905→814）,
  虽快 9% 但UI标签文字丢失不可接受（"设置"→"设"）。
  H=40 保留为可选优化，加 `--rec-height=40` 配置项待实现
...
经过 4 维度 review (Reuse/Simplification/Efficiency/Altitude) 并修复:

- OCR 响应类型 `Result`/`Response` 提取到 `pkg/ocr/ocr.go`, RPC 和 CLI 共同引用
- 删除死代码: `mergeBoxes` (80行 O(n²)), `softmax1D` (26行), `libPath` 字段
- `decodePNG` 单行 wrapper 内联
- bubble sort → `sort.Slice`, 18→10 行, O(n²)→O(n log n)
- `vision_darwin.go` 清除仅用于 `var _` guard 的无用 import
- 冗余 nil 检查 (`len==0` 后 `T==0`) 删除
- `pixelChannel` 虚调用 → 直接 `Pix` 数组访问 (det + rec 前处理)

### 进一步优化验证（CTC + 分配）

针对 rec 批量解码路径做了微优化，但实测收益为零，记录以避免重复尝试：

- CTC `decode`: `[]string` append + `strings.Join` → `strings.Builder` 单 pass
- CTC `decodeFlat`: 直接对 flat logits `[B*T*nClass]` 做 stride 解码，
  省 `recognizeBoxesBatch` 里每 box 的 `make([][]float32, T)`（15 box × 40 = 600 次 slice header 分配）

benchmark 结果（Apple M4 Pro, 单 box 40 timestep）:
```
decode_old:      277µs/op, 3 allocs/op
decodeFlat_new:  277µs/op, 3 allocs/op   (allocs 来自 Builder.grow, 不可避免)
```

结论: CTC 解码仅占全流程 4ms/110ms (3.6%), 即便零分配也无法改变端到端。
**真正瓶颈仍是 rec ONNX 推理 63ms (52%)，已被 brew ORT 无 BLAS 限制锁死。**
...
| FP16 | ORT �关 | ncnn `set_use_fp16_arithmetic(opt, 1)` ARM NEON |
...
实测分析保留（代码已从仓库移除, 仅留评估记录）: `mattn/go-tflite` 需 `libtensorflowlite_c.dylib`, 两大障碍:

1. **运行时 C 库**: `mattn/go-tflite` 需 `libtensorflowlite_c.dylib`（老 TFLite C API, `TfLiteModel*` 符号）。macOS arm64 无 prebuilt:
   - Homebrew 无 formula
   - `ai-edge-litert` pip 包用新 LiteRT C API（`libLiteRt.dylib`）, 0 个 `TfLiteModel` 符号, 不兼容
   - 唯一途径: bazel 从 TF 源码构建 `//tensorflow/lite/c:tensorflowlite_c`（数小时）
2. **模型转换**: ONNX→TFLite 所有路径（onnx2tf / onnx-tf）都依赖完整 tensorflow pip 包（~600MB）

对比: NCNN 链路（brew + pnnx, 轻量）vs TFLite 链路（bazel + tensorflow, 重型）。从 Go 部署角度, NCNN 远比 TFLite 实用。

#### MNN — 工具链通, runSession 算子兼容失败（已评估, 代码已清理）

实测分析保留（C++ shim `mnn_bridge.cpp` 等代码已从仓库移除, 仅留评估记录）。MNN 与 NCNN/TFLite 不同: **无 C API, 纯 C++**, 需写 C++ shim 暴露 C 接口给 CGO。

- 安装: brew 无 mnn formula; 但 GitHub release 有 macOS universal（arm64+x86）预编译 `MNN.framework`（含 CPU+Metal+CoreML）, + PyPI `MNN` wheel（含 `mnnconvert` 转换工具）
- 模型转换: `mnnconvert -f ONNX --modelFile rec_fixed.onnx --MNNModel rec.mnn --fp16`, 转换成功
- Go 集成: C++ shim（`Interpreter`/`Tensor`/`Session`）, build tag `mnn`, 链接 `-framework MNN`

**障碍（实测确认, 非配置能解）**: runSession 返回 `COMPUTE_SIZE_ERROR=3`, 报 `Compute Shape Error for x___tr4conv2d_97`。即使固定 ONNX input shape `[1,3,48,320]`（PP-OCR rec 原始是 dynamic batch+H+W）、切 CAFFE/TENSORFLOW 维度序、fp32 + optimizeLevel 0、用 3.x 推荐的 RuntimeInfo API, 都不解决。MNN 3.6 对 PP-OCR v3 rec 的某个 conv2d 算子无法推断输出 shape。

对比 NCNN 的 `pnnx` 转换能正确处理 PP-OCR rec 的算子（已跑通且快 28%）, MNN 的 `mnnconvert` 转换虽成功但运行时算子 shape 推断失败。这是 MNN 模型兼容性问题, 非工具链（安装/转换/加载/set input 全通）。

#### tfgo（galeone/tfgo + brew libtensorflow）— 运行时 C 库解决, 转换墙（已评估, 代码已清理）

实测分析保留（代码已从仓库移除, 仅留评估记录）。和 TFLite 不同, tfgo 链接**完整 TensorFlow C 库**（非 TFLite C API）, brew `libtensorflow` 2.21.0 提供（含 `TF_LoadSessionFromSavedModel`/`TF_NewGraph`/`TF_NewTensor` 符号）。**TFLite 的死穴（macOS arm64 无 libtensorflowlite_c prebuilt）在 tfgo 这不存在**。

- 运行时: `brew install libtensorflow`（完整 TF C API, 一键装, 不需 bazel）✅
- Go 集成: `galeone/tfgo` binding（forked tensorflow-go, pinned TF 2.9.1, 2022, 维护有限无 release）。binding 写给 2.9.1 但能链接 brew 2.21.0（核心 C API 跨版本稳定）, 编译链接验证通过 ✅
- 编译: binding 不自带 TF 头（go mod 不拉完整 TF 源码树）, 需 `CGO_CFLAGS=-I/opt/homebrew/include` 指 brew header
- 模型格式: SavedModel（非 ONNX）, 需 onnx2tf 转换

**障碍（模型转换, 实测确认）**: onnx2tf 2.6.4 对 PP-OCR v3 rec 转换失败, 两条路径都败:
- flatbuffer_direct 模式（默认, 用 ai-edge-litert）: `ReduceMean` axis 推断 bug（`axis=2 rank=1`）, 即使固定 ONNX input shape `[1,3,48,320]`
- tf_converter 模式（需装 tensorflow ~600MB + tf_keras）: `tf.transpose` 失败（dynamic shape 残留致标量 tensor, `Dimension must be 0 but is 5`）
- `-ois x,1,3,48,320` 显式 shape 覆盖也不解决

运行时（C 库 + 编译链接）全通, 但无 SavedModel 无法跑 benchmark。对比 NCNN 的 `pnnx` 能转换 PP-OCR rec, onnx2tf 对该模型的 reduce/transpose 算子有兼容 bug。
...
**phonefast 实施**：`ElementsForLLMWithViewport` 函数，传入 `screenW/screenH`，将屏外交互元素折叠为 `[off-screen] N interactive items: "..."`。

```go
// internal/format/format.go
func ElementsForLLMWithViewport(elements, maxShow, isSummary, screenW, screenH) string {
    // 屏外元素分离 → 折叠为摘要
...
**仅在 summary 模式生效**——full 模式保留全部元素，不截取。
...
**phonefast 实施**：`IsObfuscatedID` 函数，4 种格式（flat/simplexml/flatref/yml）全部应用：
...
实测（蓝牙设置页，44 个元素，14 个在屏外）：

```
优化前 (44 行全部列出):
  [0] id="0_resource_name_obfuscated" (FrameLayout) ...
  ... (44 行)

优化后 summary 模式 (21 行 + 1 摘要):
  Interactive elements on screen (21 visible of 44 total, 488x1080):
  [0] desc="返回" (IconBtn) [clickable] bounds=[12,80][124,192]
  ...
  [off-screen] 14 interactive items: "计算机", "MBP-HG966364P9-0252", ...
```
